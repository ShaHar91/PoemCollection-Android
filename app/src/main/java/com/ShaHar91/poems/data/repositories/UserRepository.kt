...
object UserRepository : BaseRepository() {
    ...
    init {
        currentUser = userDao.findFirstById(hawkCurrentUserId)
    }
    ...
    fun getCurrentUser(onSuccess: () -> Unit, onError: (Throwable) -> Unit) {
        addCall(ApiCallsManager.getCurrentUser().observeOn(AndroidSchedulers.mainThread()).subscribe({
            ...
            currentUser = userDao.createOrUpdateObjectFromJson(User::class.java, it.data.toString())
            hawkCurrentUserId = currentUser?.id ?: ""
        }, {
            onError(it)
        }, {
            onSuccess()
