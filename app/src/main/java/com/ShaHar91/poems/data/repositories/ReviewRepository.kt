...
class ReviewRepository(
    ...
    override suspend fun fetchOwnReviewForPoem(poemId: String) {
        val reviewResponse = doCall(protectedService.fetchOwnReviewForPoem(poemId, HawkManager.currentUserId))

        if (reviewResponse.isNotEmpty()) {
            reviewDao.insertMany(reviewResponse.map { it.toEntity() })
        } else {
    ...
    override suspend fun createReview(poemId: String, reviewText: String, reviewRating: Float) {
        val review = doCall(protectedService.createReview(poemId, reviewText, reviewRating))
    ...
    override suspend fun updateReview(reviewId: String, reviewText: String, reviewRating: Float) {
        val review = doCall(protectedService.editReview(reviewId, reviewText, reviewRating))
        reviewDao.insert(review.toEntity())

    }

    override suspend fun deleteReview(reviewId: String) {
        doCall(protectedService.deleteReview(reviewId))
